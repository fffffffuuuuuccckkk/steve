...
class StableST(nn.Module):
    ...
    def __init__(
        ...
        if (
            (self.fpem_use_hard_environment_router or self.fpem_conservative_inv_override)
            and not self.fpem_use_load_level_experts
        ):
            raise ValueError(
                "hard/conservative environment router requires fixed load-level experts"
            )
        if (
    ...
    def _predict_observable_load_level_experts(
        ...
        y_env_selected = select_load_expert(y_env_heads, load_level)
        router_z_inv = z_inv.detach() if self.fpem_conservative_inv_override else z_inv
        router_e_useful = e_useful.detach() if self.fpem_conservative_inv_override else e_useful
