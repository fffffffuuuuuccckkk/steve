...
summary_metric_values() {
  ...
  if [ -z "$summary_json" ] || [ ! -f "$summary_json" ]; then
    printf 'NA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA'
    return
  fi
  "$PYTHON" - "$summary_json" <<'PY'
import json, math, sys
