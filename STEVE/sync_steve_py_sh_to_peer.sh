...
if truthy "$RUN_COMPILE_CHECK"; then
  ...
  "${ssh_cmd[@]}" "$PEER" "cd '$PEER_DIR' && py='${PYTHON_BIN}' && if [ -z \"\$py\" ]; then for candidate in /data/OuXiaoyu/miniconda3/envs/basicts/bin/python /data/OuXiaoyu/miniconda3/envs/tslib/bin/python \$(command -v python3 2>/dev/null) \$(command -v python 2>/dev/null); do if [ -n \"\$candidate\" ] && [ -x \"\$candidate\" ]; then py=\"\$candidate\"; break; fi; done; fi && echo \"[sync-steve] remote python=\$py\" && \"\$py\" -m compileall -q run_tds_nyctaxi.py train.py test.py models scripts"
fi
