...
class TDSDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.observable_mode:
            ...
            self.c = None if c is None else torch.from_numpy(as_float32_array(c))
            self.observable_load_profile = None
            self.observable_load_score = None
            self.cached_load_level = None
        self.include_period_context = bool(include_period_context)
        if self.include_period_context:
            expected = self.x.shape[0]
            if (
                period_context_day is None
                or period_context_week is None
                or period_context_valid is None
            ):
                raise ValueError(
                    "include_period_context=true requires precomputed chronological "
                    "period_context_day/week/valid arrays"
                )
            self.period_context_day = torch.from_numpy(as_float32_array(period_context_day))
            self.period_context_week = torch.from_numpy(as_float32_array(period_context_week))
            self.period_context_valid = torch.from_numpy(as_float32_array(period_context_valid))
            if (
                self.period_context_day.shape != self.x.shape
                or self.period_context_week.shape != self.x.shape
                or self.period_context_valid.shape[0] != expected
                or self.period_context_valid.shape[-1] != 2
            ):
                raise ValueError(
                    "period context arrays must match x shape and valid shape [B,2]; "
                    f"got day={tuple(self.period_context_day.shape)} "
                    f"week={tuple(self.period_context_week.shape)} "
                    f"valid={tuple(self.period_context_valid.shape)} "
                    f"x={tuple(self.x.shape)}"
                )
        else:
            self.period_context_day = None
            self.period_context_week = None
            self.period_context_valid = None
    ...
    def __getitem__(self, idx):
        if self.observable_mode and self.include_time:
            ...
            if self.c is None:
                item = (self.x[idx], self.y[idx], self.time_label[idx])
            else:
        ...
        if self.include_period_context:
            item = item + (
                self.period_context_day[idx],
                self.period_context_week[idx],
                self.period_context_valid[idx],
            )
...
def make_loader(
    ...
    dataset = TDSDataset(
        ...
        include_period_context=include_period_context,
...
def unpack_tds_batch(batch):
    ...
    if (
        len(items) >= 3
        and torch.is_tensor(items[-1])
        and items[-1].dtype in (torch.int8, torch.int16, torch.int32, torch.int64, torch.long)
        and items[-1].dim() == 1
    ):
        sample_index = items.pop()
    ...
    if (
        len(items) >= 5
        and torch.is_tensor(items[-3])
        and torch.is_tensor(items[-2])
        and torch.is_tensor(items[-1])
        and items[-3].dim() == 4
        and items[-2].dim() == 4
        and items[-1].dim() == 2
        and items[-1].shape[-1] == 2
    ):
        period_context_day, period_context_week, period_context_valid = items[-3:]
        items = items[:-3]
    batch = tuple(items)
    if len(batch) == 6:
        (
    ...
    if len(batch) == 4:
        ...
        if torch.is_tensor(third) and third.dtype in (torch.int8, torch.int16, torch.int32, torch.int64, torch.long) and third.dim() == 1:
            time_label = third
            stored_c = None
        else:
            time_label = None
        ...
        raise ValueError(f"unsupported TDS batch format with {len(batch)} tensors after metadata parsing")
    return {
        "data": data,
        "target": target,
        "time_label": time_label,
...
def infer_period_context_steps(args):
    dataset = str(getattr(args, "dataset", "")).lower()
    if "largest" in dataset:
        auto_day = 288
    elif "nyctaxi" in dataset or "nyc" in dataset:
        auto_day = 48
    else:
        auto_day = 24
    day_steps = int(getattr(args, "fpem_env_period_context_day_steps", 0) or auto_day)
    week_steps = int(getattr(args, "fpem_env_period_context_week_steps", 0) or day_steps * 7)
    return max(day_steps, 1), max(week_steps, 1)


def attach_period_context_to_pack(pack, day_steps, week_steps):
    """Attach same-time previous-day/week windows before any subsetting.

    The pack is assumed to be in chronological sample order. We compute the
    lagged windows here so later workday/holiday sampling or test subsetting
    cannot corrupt the temporal reference. Missing early history is filled by
    the current x window and marked invalid in period_context_valid.
    """
    x = np.asarray(pack["x"], dtype=np.float32)
    n = int(x.shape[0])
    day_steps = max(int(day_steps), 1)
    week_steps = max(int(week_steps), 1)
    idx = np.arange(n, dtype=np.int64)
    day_valid = idx >= day_steps
    week_valid = idx >= week_steps
    day_idx = idx - day_steps
    week_idx = idx - week_steps
    day_context = x.copy()
    week_context = x.copy()
    if day_valid.any():
        day_context[day_valid] = x[day_idx[day_valid]]
    if week_valid.any():
        week_context[week_valid] = x[week_idx[week_valid]]
    pack["period_context_day"] = np.ascontiguousarray(day_context, dtype=np.float32)
    pack["period_context_week"] = np.ascontiguousarray(week_context, dtype=np.float32)
    pack["period_context_valid"] = np.stack(
        [
            day_valid.astype(np.float32),
            week_valid.astype(np.float32),
        ],
        axis=-1,
    )
    return {
        "day_coverage": float(day_valid.mean()) if n else 0.0,
        "week_coverage": float(week_valid.mean()) if n else 0.0,
    }
...
def build_tds_data(args):
    ...
    if use_observable_prior and not ignore_future_c:
        raise ValueError(
            "fpem_use_observable_load_prior requires fpem_ignore_future_c=true"
        )
    exclude_stored_future_c = bool(use_observable_prior or ignore_future_c)
    use_period_context = bool(getattr(args, "fpem_env_use_period_context", False))
    period_day_steps, period_week_steps = infer_period_context_steps(args)
    args.fpem_env_period_context_day_steps_resolved = int(period_day_steps)
    args.fpem_env_period_context_week_steps_resolved = int(period_week_steps)
    if use_period_context:
        print(
            "[period-context] resolved "
            f"dataset={getattr(args, 'dataset', '')} "
            f"day_steps={period_day_steps} week_steps={period_week_steps}; "
            "computed before subset; invalid_fill=current_input",
            flush=True,
        )
    raw = {
        cat: load_npz(
            os.path.join(root, f"{cat}.npz"),
            include_future_c=not exclude_stored_future_c,
        )
    ...
    if use_period_context:
        for split in ("train", "val", "test"):
            period_context_summary[split] = attach_period_context_to_pack(
                raw[split], period_day_steps, period_week_steps
            )
    ...
    for pack in packs.values():
        ...
        if "period_context_day" in pack:
            pack["period_context_day"] = standard_transform_float32(
                pack["period_context_day"], scaler
            )
        if "period_context_week" in pack:
            pack["period_context_week"] = standard_transform_float32(
                pack["period_context_week"], scaler
            )
        if "c" in pack:
    ...
    def loader_for(name, batch_size, shuffle, drop_last):
        ...
        return make_loader(
            ...
            drop_last=drop_last,
            pin_memory=pin_memory,
            include_period_context=use_period_context,
            period_context_day=pack.get("period_context_day"),
            period_context_week=pack.get("period_context_week"),
            period_context_valid=pack.get("period_context_valid"),
            **observable_kwargs,
        )
    ...
    counts = {
        ...
        "fpem_env_use_period_context": bool(use_period_context),
        "fpem_env_period_context_day_steps": int(period_day_steps),
        "fpem_env_period_context_week_steps": int(period_week_steps),
        "fpem_env_period_context_train_day_coverage": float(
            period_context_summary.get("train", {}).get("day_coverage", 0.0)
        ),
        "fpem_env_period_context_train_week_coverage": float(
            period_context_summary.get("train", {}).get("week_coverage", 0.0)
        ),
        "fpem_env_period_context_val_day_coverage": float(
            period_context_summary.get("val", {}).get("day_coverage", 0.0)
        ),
        "fpem_env_period_context_val_week_coverage": float(
            period_context_summary.get("val", {}).get("week_coverage", 0.0)
        ),
        "fpem_env_period_context_test_day_coverage": float(
            period_context_summary.get("test", {}).get("day_coverage", 0.0)
        ),
        "fpem_env_period_context_test_week_coverage": float(
            period_context_summary.get("test", {}).get("week_coverage", 0.0)
        ),
        "fpem_env_period_context_computed_before_subset": bool(use_period_context),
        "fpem_env_period_context_index_source": "raw chronological split before train/test subsetting" if use_period_context else "disabled",
        "fpem_env_period_context_invalid_fill": "current_input" if use_period_context else "disabled",
    }
    del packs
    gc.collect()
...
class AGCRNForecast(nn.Module):
    def __init__(self, args):
        ...
        self.horizon = int(getattr(args, "output_T_dim", 1))
...
def build_model(args, graph):
    if args.model == "agcrn":
        ...
        model = StableST(
            ...
            output_T_dim=int(getattr(args, "output_T_dim", 1)),
...
def weighted_flow_mae(pred, target, scaler, yita=0.5):
    ...
    if pred.shape[-1] == 1 or target.shape[-1] == 1:
        return mae_torch(pred[..., 0], target[..., 0], mask_value=5.0)
...
def _clear_eval_runtime_cache(model):
    if hasattr(model, "clear_fpem_runtime_cache"):
        model.clear_fpem_runtime_cache()
    if hasattr(model, "_last_eval_output"):
        model._last_eval_output = None


class StreamingMaskedMetrics:
    """CPU streaming equivalent of test_metrics(pred, true, mask1=5, mask2=5)."""

    def __init__(self, mask1=5.0, mask2=5.0):
        self.mask1 = float(mask1)
        self.mask2 = float(mask2)
        self.mae_sum = 0.0
        self.mape_sum = 0.0
        self.mae_count = 0
        self.mape_count = 0
    ...
    def update(self, pred, true):
        ...
        mae_mask = true > self.mask1
        if bool(mae_mask.any()):
            self.mae_sum += torch.abs(true[mae_mask] - pred[mae_mask]).sum().item()
            self.mae_count += int(mae_mask.sum().item())
        mape_mask = true > self.mask2
        if bool(mape_mask.any()):
            self.mape_sum += torch.abs((true[mape_mask] - pred[mape_mask]) / true[mape_mask]).sum().item()
            self.mape_count += int(mape_mask.sum().item())

    def result(self):
        mae = self.mae_sum / self.mae_count if self.mae_count else float("nan")
        mape = self.mape_sum / self.mape_count if self.mape_count else float("nan")
        return float(mae), float(mape)


def evaluate(model, loader, scaler, args, max_batches=None):
    model.eval()
    metrics = StreamingMaskedMetrics()
    ...
    with torch.inference_mode():
        for batch_idx, raw_batch in enumerate(loader):
            ...
            count += 1
            pred_raw = scaler.inverse_transform(pred.squeeze(1)).detach().cpu()
            target_raw = scaler.inverse_transform(target.squeeze(1)).detach().cpu()
            metrics.update(pred_raw, target_raw)
            _clear_eval_runtime_cache(model)
            del batch, pred, target, pred_raw, target_raw
    mae, mape = metrics.result()
...
def evaluate_steve_output(model, loader, scaler, args, output_key, max_batches=None):
    ...
    with torch.inference_mode():
        for batch_idx, raw_batch in enumerate(loader):
            ...
            prediction_raw = scaler.inverse_transform(prediction.squeeze(1)).detach().cpu()
            target_raw = scaler.inverse_transform(unpacked["target"].squeeze(1)).detach().cpu()
            metrics.update(prediction_raw, target_raw)
            _clear_eval_runtime_cache(model)
            del batch, unpacked, output, prediction, prediction_raw, target_raw
    mae, mape = metrics.result()
...
MECHANISM_SUMMARY_KEYS = {
    ...
    "counterfactual_risk_all_samples_used_for_router": "fpem/counterfactual_risk_all_samples_used_for_router",
    "counterfactual_risk_deploy_route_active": "fpem/counterfactual_risk_deploy_route_active",
    "counterfactual_risk_training_uses_route": "fpem/counterfactual_risk_training_uses_route",
    ...
    "confounder_dep_enabled": "fpem/confounder_dep_enabled",
    "confounder_dep_total": "fpem/confounder_dep_total",
    "confounder_loss_gci": "fpem/loss_gci",
    "confounder_loss_gci_drop": "fpem/loss_gci_drop",
    "confounder_loss_gci_keep": "fpem/loss_gci_keep",
    "confounder_loss_gci_graph_align": "fpem/loss_gci_graph_align",
    "gci_graph_align_raw_mse": "fpem/gci_graph_align_raw_mse",
    "gci_graph_align_normalized": "fpem/gci_graph_align_normalized",
    "confounder_loss_scd": "fpem/loss_scd",
    "confounder_loss_kl": "fpem/loss_conf_kl",
    "confounder_effective_gci_weight": "fpem/effective_gci_weight",
    "confounder_effective_gci_graph_align_weight": "fpem/effective_gci_graph_align_weight",
    "confounder_effective_scd_weight": "fpem/effective_scd_weight",
    "confounder_effective_kl_weight": "fpem/effective_conf_kl_weight",
    "confounder_dep_ramp_factor": "fpem/confounder_dep_ramp_factor",
    "confounder_C_mean": "fpem/C_mean",
    "confounder_C_std": "fpem/C_std",
    "confounder_C_norm": "fpem/C_norm",
    "confounder_projection_trace": "fpem/confounder_projection_trace",
    "confounder_projection_ridge": "fpem/confounder_projection_ridge",
    "confounder_projection_beta_norm": "fpem/confounder_projection_beta_norm",
    "confounder_centered_c_norm": "fpem/confounder_centered_c_norm",
    "confounder_centered_z_norm": "fpem/confounder_centered_z_norm",
    "z_before_norm": "fpem/z_before_norm",
    "z_after_norm": "fpem/z_after_norm",
    "z_before_after_diff_norm": "fpem/z_before_after_diff_norm",
    "confounder_dep_detach_target": "fpem/confounder_dep_detach_target",
    "physical_batch_size": "fpem/physical_batch_size",
    "confounder_virtual_batch_enabled": "fpem/confounder_virtual_batch_enabled",
    "confounder_virtual_batch_target_size": "fpem/confounder_virtual_batch_target_size",
    "confounder_virtual_batch_actual_size": "fpem/confounder_virtual_batch_actual_size",
    "confounder_queue_size": "fpem/confounder_queue_size",
    "gci_nonedge_mass_before_C": "fpem/gci_nonedge_mass_before_C",
    "gci_nonedge_mass_after_C": "fpem/gci_nonedge_mass_after_C",
    "gci_nonedge_mass_reduction": "fpem/gci_nonedge_mass_reduction",
    "gci_relative_violation": "fpem/gci_relative_violation",
    "gci_edge_before_C": "fpem/gci_edge_before_C",
    "gci_edge_after_C": "fpem/gci_edge_after_C",
    "gci_nonedge_conn_before_C": "fpem/gci_nonedge_conn_before_C",
    "gci_nonedge_conn_after_C": "fpem/gci_nonedge_conn_after_C",
    "gci_nonedge_conn_reduction": "fpem/gci_nonedge_conn_reduction",
    "gci_edge_conn_before_C": "fpem/gci_edge_conn_before_C",
    "gci_edge_conn_after_C": "fpem/gci_edge_conn_after_C",
    "gci_edge_conn_reduction": "fpem/gci_edge_conn_reduction",
    "gci_nonedge_dep_before_C": "fpem/gci_nonedge_dep_before_C",
    "gci_nonedge_dep_after_C": "fpem/gci_nonedge_dep_after_C",
    "gci_nonedge_dep_reduction": "fpem/gci_nonedge_dep_reduction",
    "gci_edge_dep_before_C": "fpem/gci_edge_dep_before_C",
    "gci_edge_dep_after_C": "fpem/gci_edge_dep_after_C",
    "scd_assoc_before_C": "fpem/scd_assoc_before_C",
    "scd_assoc_after_C": "fpem/scd_assoc_after_C",
    "scd_assoc_reduction": "fpem/scd_assoc_reduction",
    "scd_relative_violation": "fpem/scd_relative_violation",
    "sample_assoc_before_C": "fpem/sample_assoc_before_C",
    "sample_assoc_after_C": "fpem/sample_assoc_after_C",
    "sample_assoc_reduction": "fpem/sample_assoc_reduction",
    "sample_dep_before_C": "fpem/sample_dep_before_C",
    "sample_dep_after_C": "fpem/sample_dep_after_C",
    "sample_dep_reduction": "fpem/sample_dep_reduction",
    "gci_node_vector_dim": "fpem/gci_node_vector_dim",
    "gci_graph_embed_dim": "fpem/gci_graph_embed_dim",
    "gci_graph_embedding_norm_before": "fpem/gci_graph_embedding_norm_before",
    "gci_graph_embedding_norm_after": "fpem/gci_graph_embedding_norm_after",
    "gci_graph_embedding_std_before": "fpem/gci_graph_embedding_std_before",
    "gci_graph_embedding_std_after": "fpem/gci_graph_embedding_std_after",
    "gci_graph_adj_mean_before": "fpem/gci_graph_adj_mean_before",
    "gci_graph_adj_mean_after": "fpem/gci_graph_adj_mean_after",
    "gci_graph_adj_std_before": "fpem/gci_graph_adj_std_before",
    "gci_graph_adj_std_after": "fpem/gci_graph_adj_std_after",
    "gci_graph_align_weight": "fpem/gci_graph_align_weight",
    "gci_edge_preserve_beta": "fpem/gci_edge_preserve_beta",
    "gci_graph_align_target_available": "fpem/gci_graph_align_target_available",
    "gci_target_adj_mean": "fpem/gci_target_adj_mean",
    "gci_target_adj_std": "fpem/gci_target_adj_std",
    "gci_target_edge_mean": "fpem/gci_target_edge_mean",
    "gci_target_nonedge_mean": "fpem/gci_target_nonedge_mean",
    "gci_target_diag_mean": "fpem/gci_target_diag_mean",
    "gci_graph_align_updates_agcrn": "fpem/gci_graph_align_updates_agcrn",
    "gci_updates_graph_linear": "fpem/gci_updates_graph_linear",
    "gci_graph_construction_agcrn_softmax": "fpem/gci_graph_construction_agcrn_softmax",
    "scd_sample_vector_dim": "fpem/scd_sample_vector_dim",
    ...
    "fpem_env_use_period_context": "fpem/fpem_env_use_period_context",
    "env_period_context_available": "fpem/env_period_context_available",
    "env_period_day_available_ratio": "fpem/env_period_day_available_ratio",
    "env_period_week_available_ratio": "fpem/env_period_week_available_ratio",
    "env_period_context_feature_dim": "fpem/env_period_context_feature_dim",
    "env_period_context_scale": "fpem/env_period_context_scale",
    "env_period_context_delta_norm": "fpem/env_period_context_delta_norm",
    "env_period_context_day_diff_norm": "fpem/env_period_context_day_diff_norm",
    "env_period_context_week_diff_norm": "fpem/env_period_context_week_diff_norm",
...
def parse_args():
    ...
    parser.add_argument("--fpem_env_use_period_context", type=str2bool, default=False)
    parser.add_argument(
        "--fpem_env_period_context_day_steps",
        type=int,
        default=0,
        help="0 means auto: 48 for NYCTaxi 30-minute data, 288 for LargeST 5-minute data, 24 otherwise",
    )
    parser.add_argument(
        "--fpem_env_period_context_week_steps",
        type=int,
        default=0,
        help="0 means 7 * resolved day_steps",
    )
    parser.add_argument("--fpem_env_period_context_scale", type=float, default=0.1)
    ...
    parser.add_argument("--confounder_dep_mode", choices=["none", "gci", "scd", "both"], default="none")
    parser.add_argument("--confounder_extractor", choices=["token_attention"], default="token_attention")
    parser.add_argument("--confounder_dim", type=int, default=8)
    parser.add_argument("--confounder_num_heads", type=int, default=4)
    parser.add_argument("--confounder_attention_dropout", type=float, default=0.0)
    parser.add_argument("--confounder_graph_embed_dim", type=int, default=16)
    parser.add_argument("--confounder_variational", type=str2bool, default=True)
    parser.add_argument("--confounder_kl_weight", type=float, default=0.0001)
    parser.add_argument("--confounder_dep_warmup_epochs", type=int, default=5)
    parser.add_argument("--confounder_dep_ramp_epochs", type=int, default=10)
    parser.add_argument("--confounder_projection_ridge", type=float, default=0.001)
    parser.add_argument("--confounder_dep_detach_target", type=str2bool, default=True)
    parser.add_argument("--confounder_virtual_batch_enabled", type=str2bool, default=False)
    parser.add_argument("--confounder_virtual_batch_size", type=int, default=8)
    parser.add_argument("--gci_weight", type=float, default=0.001)
    parser.add_argument("--gci_graph_align_weight", type=float, default=0.1)
    parser.add_argument("--gci_edge_preserve_beta", type=float, default=0.1)
    parser.add_argument("--gci_graph_hops", type=int, default=1)
    parser.add_argument("--gci_symmetrize_adj", type=str2bool, default=True)
    parser.add_argument("--gci_add_self_loops", type=str2bool, default=True)
    parser.add_argument("--scd_weight", type=float, default=0.001)
    parser.add_argument("--gci_ridge", type=float, default=None, help=argparse.SUPPRESS)
    parser.add_argument("--scd_ridge", type=float, default=None, help=argparse.SUPPRESS)
    parser.add_argument("--gci_detach_target", type=str2bool, default=None, help=argparse.SUPPRESS)
    parser.add_argument("--scd_detach_target", type=str2bool, default=None, help=argparse.SUPPRESS)
    parser.add_argument("--scd_alignment_weight", type=float, default=None, help=argparse.SUPPRESS)
    parser.add_argument("--dep_eps", type=float, default=1.0e-6)
    parser.add_argument("--confounder_injection", choices=["none"], default="none")
    ...
    cli_values = vars(args_cli).copy()
    argv = sys.argv[1:]

    def option_was_provided(option_name):
        return any(item == option_name or item.startswith(option_name + "=") for item in argv)

    # Preserve dataset selection from dataset-specific YAML files unless the
    # caller explicitly overrides it. Historical launch scripts pass these
    # fields, so their behavior remains unchanged.
    for key, option_name in [
        ...
        if key in cfg and not option_was_provided(option_name):
