...
export RUN_PREFIX=${RUN_PREFIX:-fpem_agcrn_aligned_pretrained_inv_module_build}
...
export RUN_LEAVE_ONE_OUT=${RUN_LEAVE_ONE_OUT:-true}
export RUN_BUILD_UP=${RUN_BUILD_UP:-true}
export RUN_CUMULATIVE_BUILD=${RUN_CUMULATIVE_BUILD:-false}
...
# True invariant-only baseline. The extractor is also disabled because the
# environment/confounder branch must not consume memory without affecting prediction.
INV_ONLY_ARGS=(
...
# Build-up is not leave-one-out: start with no FPEM correction, then add only
# the named module. Auxiliary losses need route/fusion plus C_cur as a carrier.
# The old temporal-pooling correction explicitly disables EnvMask so that
# bu_env_correction_only contains no hidden auxiliary module.
BU_ENV_CORRECTION_ARGS=(
  --fpem_use_env_route true
  --fpem_use_env_fusion true
  --fpem_use_env_mask false
  --fpem_env_route_k 1
...
# loo_no_confounder_extractor does not remove the whole environment branch.
# It replaces the basis-style confounder extractor with the previous
# temporal-pooling environment representation path.
run_named_ablation() {
  local gpu_id="$1" name="$2" seed="$3"
  case "$name" in
    loo_full) run_one "$gpu_id" "$name" "$seed" ;;
    loo_inv_only) run_one "$gpu_id" "$name" "$seed" "${INV_ONLY_ARGS[@]}" ;;
    loo_k3) run_one "$gpu_id" "$name" "$seed" \
      --fpem_env_route_k 3 --fpem_env_route_lambda_balance 0.01 --fpem_env_route_lambda_diverse 0.01 ;;
    loo_with_inv_loss) run_one "$gpu_id" "$name" "$seed" --fpem_lambda_inv_pred 0.2 ;;
    loo_no_future_mi) run_one "$gpu_id" "$name" "$seed" \
      --fpem_use_future_mi false --fpem_lambda_future_mi 0.0 ;;
    loo_no_swap) run_one "$gpu_id" "$name" "$seed" \
      --fpem_use_swap false --fpem_lambda_swap 0.0 ;;
    loo_no_club) run_one "$gpu_id" "$name" "$seed" \
      --fpem_use_club_mi false --fpem_lambda_club_mi 0.0 ;;
    loo_no_confounder_extractor) run_one "$gpu_id" "$name" "$seed" \
      --fpem_use_confounder_extractor false ;;

    bu_inv_only) run_one "$gpu_id" "$name" "$seed" "${INV_ONLY_ARGS[@]}" ;;
    bu_env_correction_only) run_one "$gpu_id" "$name" "$seed" "${BU_ENV_CORRECTION_ARGS[@]}" ;;
    bu_confounder_extractor_only) run_one "$gpu_id" "$name" "$seed" "${BU_CONFOUNDER_ARGS[@]}" ;;
    bu_club_only) run_one "$gpu_id" "$name" "$seed" "${BU_CLUB_ARGS[@]}" ;;
    bu_future_mi_only) run_one "$gpu_id" "$name" "$seed" "${BU_FUTURE_MI_ARGS[@]}" ;;
    bu_swap_only) run_one "$gpu_id" "$name" "$seed" "${BU_SWAP_ARGS[@]}" ;;
    bu_inv_loss_only) run_one "$gpu_id" "$name" "$seed" "${BU_INV_LOSS_ARGS[@]}" ;;
    bu_k3_route_only) run_one "$gpu_id" "$name" "$seed" "${BU_K3_ROUTE_ARGS[@]}" ;;

    cum_inv_only) run_one "$gpu_id" "$name" "$seed" "${INV_ONLY_ARGS[@]}" ;;
    cum_env_correction) run_one "$gpu_id" "$name" "$seed" "${BU_ENV_CORRECTION_ARGS[@]}" ;;
    cum_plus_confounder_extractor) run_one "$gpu_id" "$name" "$seed" "${BU_CONFOUNDER_ARGS[@]}" ;;
    cum_plus_club) run_one "$gpu_id" "$name" "$seed" "${BU_CLUB_ARGS[@]}" ;;
    cum_plus_future_mi) run_one "$gpu_id" "$name" "$seed" "${CUM_PLUS_FUTURE_ARGS[@]}" ;;
    cum_plus_swap) run_one "$gpu_id" "$name" "$seed" "${CUM_PLUS_SWAP_ARGS[@]}" ;;
    cum_plus_k3) run_one "$gpu_id" "$name" "$seed" "${CUM_PLUS_K3_ARGS[@]}" ;;
    cum_plus_inv_loss) run_one "$gpu_id" "$name" "$seed" "${CUM_PLUS_INV_LOSS_ARGS[@]}" ;;

    hyper_full) run_one "$gpu_id" "$name" "$seed" "${HYPER_FULL_ARGS[@]}" ;;
    hyper_no_hyper_reg) run_one "$gpu_id" "$name" "$seed" \
      "${HYPER_FULL_ARGS[@]}" --fpem_lambda_hyper_delta_norm 0.0 ;;
    hyper_no_alpha_gate) run_one "$gpu_id" "$name" "$seed" \
      "${HYPER_FULL_ARGS[@]}" --fpem_hyper_alpha_mode fixed_one ;;
    hyper_no_swap_fallback) run_one "$gpu_id" "$name" "$seed" \
      "${HYPER_FULL_ARGS[@]}" --fpem_use_swap_fallback_router_loss false --fpem_lambda_swap_fallback_router 0.0 ;;
    *) echo "[ERROR] unknown ablation: $name" >&2; return 2 ;;
  esac
}

LOO_NAMES=(
  loo_full loo_inv_only loo_k3 loo_with_inv_loss loo_no_future_mi loo_no_swap
  loo_no_club loo_no_confounder_extractor
)
BUILD_UP_NAMES=(
  bu_inv_only bu_env_correction_only bu_confounder_extractor_only bu_club_only
  bu_future_mi_only bu_swap_only bu_inv_loss_only bu_k3_route_only
)
CUMULATIVE_NAMES=(
  cum_inv_only cum_env_correction cum_plus_confounder_extractor cum_plus_club
  cum_plus_future_mi cum_plus_swap cum_plus_k3 cum_plus_inv_loss
)
HYPER_NAMES=(
  hyper_full hyper_no_hyper_reg hyper_no_alpha_gate hyper_no_swap_fallback
)
...
truthy "$RUN_LEAVE_ONE_OUT" && add_group_jobs "${LOO_NAMES[@]}"
truthy "$RUN_BUILD_UP" && add_group_jobs "${BUILD_UP_NAMES[@]}"
truthy "$RUN_CUMULATIVE_BUILD" && add_group_jobs "${CUMULATIVE_NAMES[@]}"
truthy "$RUN_HYPER_ABLATIONS" && add_group_jobs "${HYPER_NAMES[@]}"
...
echo "RUN_LEAVE_ONE_OUT=$RUN_LEAVE_ONE_OUT"
echo "RUN_BUILD_UP=$RUN_BUILD_UP"
echo "RUN_CUMULATIVE_BUILD=$RUN_CUMULATIVE_BUILD"
