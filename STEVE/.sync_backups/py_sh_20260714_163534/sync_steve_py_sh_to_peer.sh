...
if truthy "$RUN_COMPILE_CHECK"; then
  ...
  "${ssh_cmd[@]}" "$PEER" "cd '$PEER_DIR' && py='${PYTHON_BIN}' && if [ -z \"\$py\" ]; then py=\$(command -v python3 || command -v python); fi && \"\$py\" -m compileall -q run_tds_nyctaxi.py train.py test.py models scripts"
fi
