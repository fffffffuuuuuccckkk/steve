...
from models.fpem.confounder_regularization import (
...
class OriginalStableST(nn.Module):
    ...
    def __init__(
        ...
        if (not self.steve_disable_env_stream) and self.confounder_dep_mode != "none":
    ...
    def calculate_loss(
        ...
        if self.steve_disable_env_stream:
            ...
            self.latest_fpem_logs = {
                ...
                for key, value in zero_confounder_dependence_logs(
                    lp,
                    mode=self.confounder_dep_mode,
                ).items()
            }
        ...
        conf_dep_loss, conf_dep_logs = confounder_dependence_terms(
            self.args,
        ...
        if training:
            loss = loss + conf_dep_loss
        self.latest_fpem_logs = {
            key: float(value.detach().cpu().item())
            for key, value in conf_dep_logs.items()
