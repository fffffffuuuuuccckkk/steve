...
class StableST(nn.Module):
    ...
    def _load_pretrained_inv_agcrn(self):
        ...
        if end_conv_weight is None or end_conv_bias is None:
            raise ValueError(
                "checkpoint has no usable AGCRN end_conv prediction head: {}".format(path)
            )
        ...
        if mapped_weight.shape != expected.shape:
            raise ValueError(
                "AGCRN end_conv cannot map to invariant head: {} vs {}".format(
        ...
        print(
            "[StableST] loaded pretrained invariant AGCRN encoder and prediction head "
