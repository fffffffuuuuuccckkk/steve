...
class StableST(nn.Module):
    ...
    def __init__(
        ...
        self._pretrained_inv_agcrn_prediction_head_loaded = False
        ...
        self.fpem_ignore_future_c = self._as_bool(getattr(args, "fpem_ignore_future_c", False))
        self.fpem_use_observable_load_prior = self._as_bool(
        ...
        self.fpem_env_use_period_context = self._as_bool(
            getattr(args, "fpem_env_use_period_context", False)
        )
        self.fpem_env_period_context_scale = float(
            getattr(args, "fpem_env_period_context_scale", 0.1)
        )
        self.fpem_env_period_context_channels = int(getattr(args, "d_input", in_channels))
        ...
        self.fpem_use_load_level_experts = self._as_bool(
            getattr(args, "fpem_use_load_level_experts", False)
        )
        self.fpem_load_level_k = int(
            getattr(args, "fpem_load_level_k", self.fpem_env_route_k)
        )
        self.fpem_load_level_mode = str(
            getattr(args, "fpem_load_level_mode", "train_quantile")
        ).lower()
        self.fpem_randomize_train_load_level = self._as_bool(
            getattr(args, "fpem_randomize_train_load_level", False)
        )
        self.fpem_use_environment_gate = self._as_bool(
            getattr(args, "fpem_use_environment_gate", True)
        )
        self.fpem_use_hard_environment_router = self._as_bool(
            getattr(args, "fpem_use_hard_environment_router", False)
        )
        self.fpem_conservative_inv_override = self._as_bool(
            getattr(args, "fpem_conservative_inv_override", False)
        )
        self.fpem_counterfactual_risk_router = self._as_bool(
            getattr(args, "fpem_counterfactual_risk_router", False)
        )
        self.fpem_hard_router_warmup_epochs = int(
            getattr(args, "fpem_hard_router_warmup_epochs", 5)
        )
        self.fpem_environment_gate_warmup_epochs = int(
            getattr(args, "fpem_environment_gate_warmup_epochs", 5)
        )
        self.fpem_environment_gate_margin_relative = float(
            getattr(args, "fpem_environment_gate_margin_relative", 0.01)
        )
        self.fpem_environment_gate_temperature = float(
            getattr(args, "fpem_environment_gate_temperature", 0.05)
        )
        self.fpem_lambda_environment_gate = float(
            getattr(args, "fpem_lambda_environment_gate", 1.0)
        )
        self.fpem_lambda_load_expert = float(
            getattr(args, "fpem_lambda_load_expert", 0.2)
        )
        self.fpem_lambda_hard_router = float(
            getattr(args, "fpem_lambda_hard_router", 1.0)
        )
        self.fpem_override_margin = float(
            getattr(args, "fpem_override_margin", 0.0)
        )
        self.fpem_override_threshold = float(
            getattr(args, "fpem_override_threshold", 0.5)
        )
        self.fpem_override_weight_min = float(
            getattr(args, "fpem_override_weight_min", 0.0)
        )
        self.fpem_override_weight_max = float(
            getattr(args, "fpem_override_weight_max", 20.0)
        )
        self.fpem_harmful_switch_weight = float(
            getattr(args, "fpem_harmful_switch_weight", 2.0)
        )
        self.fpem_override_router_loss_weight = float(
            getattr(args, "fpem_override_router_loss_weight", 1.0)
        )
        self.fpem_counterfactual_risk_stage2_start_epoch = int(
            getattr(args, "fpem_counterfactual_risk_stage2_start_epoch", 20)
        )
        self.fpem_counterfactual_risk_regression_weight = float(
            getattr(args, "fpem_counterfactual_risk_regression_weight", 1.0)
        )
        self.fpem_counterfactual_risk_ranking_weight = float(
            getattr(args, "fpem_counterfactual_risk_ranking_weight", 0.5)
        )
        self.fpem_counterfactual_risk_router_loss_weight = float(
            getattr(args, "fpem_counterfactual_risk_router_loss_weight", 1.0)
        )
        if self.fpem_counterfactual_risk_router and (
            self.fpem_conservative_inv_override
            or self.fpem_use_hard_environment_router
        ):
            raise ValueError(
                "fpem_counterfactual_risk_router is mutually exclusive with "
                "fpem_conservative_inv_override/fpem_use_hard_environment_router"
            )
        if self.fpem_use_observable_load_prior and not self.fpem_ignore_future_c:
            raise ValueError(
                "observable-load prior mode enforces fpem_ignore_future_c=true"
            )
        if (
            (
                self.fpem_use_hard_environment_router
                or self.fpem_conservative_inv_override
                or self.fpem_counterfactual_risk_router
            )
            and not self.fpem_use_load_level_experts
        ):
            raise ValueError(
                "hard/conservative/risk router requires fixed load-level experts"
            )
        if (
            self.fpem_use_observable_load_prior
            and self.fpem_use_load_level_experts
            and str(getattr(args, "fpem_hyper_alpha_mode", "")).lower()
            != "fixed_one"
        ):
            raise ValueError(
                "observable fixed experts require fpem_hyper_alpha_mode=fixed_one"
            )
        if self.fpem_use_load_level_experts or self.fpem_ignore_future_c:
            if not self.fpem_use_env_route:
                raise ValueError("load-level experts require fpem_use_env_route=true")
            if self.fpem_load_level_k != self.fpem_env_route_k:
                raise ValueError(
                    "fpem_load_level_k must equal fpem_env_route_k for load-level experts"
                )
            if self.fpem_load_level_mode != "train_quantile":
                raise ValueError("the leakage-safe implementation currently requires train_quantile mode")
        thresholds = torch.as_tensor(
            getattr(args, "fpem_load_level_thresholds", []), dtype=torch.float32
        ).reshape(-1)
        expected_thresholds = max(self.fpem_load_level_k - 1, 0)
        if self.fpem_use_load_level_experts and thresholds.numel() != expected_thresholds:
            raise ValueError(
                "expected {} fitted load thresholds, got {}".format(
                    expected_thresholds, thresholds.numel()
                )
            )
        self.register_buffer("load_level_thresholds", thresholds)
        random_levels = torch.as_tensor(
            getattr(args, "fpem_random_train_load_levels", []), dtype=torch.long
        ).reshape(-1)
        self.register_buffer("random_train_load_levels", random_levels)
        self.fpem_input_scaler_mean = float(getattr(args, "fpem_input_scaler_mean", 0.0))
        self.fpem_input_scaler_std = float(getattr(args, "fpem_input_scaler_std", 1.0))
        self._fpem_current_epoch = 0
        self._latest_observed_load_score = None
        self._latest_load_level = None
        self._latest_observable_load_profile = None
        ...
        self.confounder_dep_mode = normalize_confounder_dep_mode(
            getattr(args, "confounder_dep_mode", "none")
        )
        setattr(self.args, "confounder_dep_mode", self.confounder_dep_mode)
        if self.confounder_dep_mode not in {"none", "gci", "scd", "both"}:
            raise ValueError("confounder_dep_mode must be one of none, gci, scd, both")
        self.confounder_extractor_type = str(
            getattr(args, "confounder_extractor", "token_attention")
        ).strip().lower()
        if self.confounder_extractor_type != "token_attention":
            raise ValueError("confounder_extractor currently supports token_attention only")
        self.confounder_dim = int(getattr(args, "confounder_dim", 8))
        self.confounder_num_heads = int(getattr(args, "confounder_num_heads", 4))
        self.confounder_attention_dropout = float(
            getattr(args, "confounder_attention_dropout", 0.0)
        )
        self.confounder_graph_embed_dim = int(
            getattr(args, "confounder_graph_embed_dim", 16)
        )
        self.confounder_variational = self._as_bool(
            getattr(args, "confounder_variational", True)
        )
        self.confounder_virtual_batch_enabled = self._as_bool(
            getattr(args, "confounder_virtual_batch_enabled", False)
        )
        self.confounder_virtual_batch_size = max(
            int(getattr(args, "confounder_virtual_batch_size", 8)),
            2,
        )
        setattr(self.args, "confounder_virtual_batch_enabled", self.confounder_virtual_batch_enabled)
        setattr(self.args, "confounder_virtual_batch_size", self.confounder_virtual_batch_size)
        self._confounder_virtual_batch_queue = {"Z": [], "C": []}
        if str(getattr(args, "confounder_injection", "none")).strip().lower() != "none":
            raise ValueError("confounder_injection currently supports none only")
        self.latent_confounder_extractor = None
        self.latent_confounder_graph_learner = None
        ...
        self.environment_use_gate = EnvironmentUseGate(
            embed_size,
            self.fpem_load_level_k,
            int(getattr(args, "fpem_environment_gate_hidden_dim", 64)),
            args.dropout,
        )
        self.hard_environment_use_router = HardEnvironmentUseRouter(
            embed_size,
            self.fpem_load_level_k,
            int(getattr(args, "fpem_hard_router_hidden_dim", 64)),
            args.dropout,
        )
        self.load_level_env_residual_scale = nn.Parameter(torch.zeros(()))
        ...
        if self.fpem_use_load_level_experts:
            with torch.no_grad():
                self.load_level_env_residual_scale.zero_()
        with torch.no_grad():
        ...
        self.load_level_env_embedding = nn.Embedding(
            self.fpem_load_level_k,
            self.fpem_env_exogenous_load_channels * self.fpem_env_exogenous_load_dim,
        )
        ...
        self.env_period_proj = nn.Sequential(
            nn.Linear(self.fpem_env_period_context_channels * 3, embed_size),
            nn.ReLU(),
            nn.Linear(embed_size, int(getattr(args, "d_input", in_channels))),
        )
        self._init_env_exogenous_parameters()
        self._env_exog_warned_missing = False
        self._env_period_warned_missing = False
        self._latest_env_exog_logs = None
        self._latest_env_period_logs = None
        if self.confounder_dep_mode != "none":
            self.latent_confounder_extractor = LatentConfounderExtractor(
                hidden_dim=embed_size,
                confounder_dim=self.confounder_dim,
                num_heads=self.confounder_num_heads,
                attention_dropout=self.confounder_attention_dropout,
                variational=self.confounder_variational,
            )
            if self.confounder_dep_mode in {"gci", "both"}:
                self.latent_confounder_graph_learner = FunctionalGraphLearner(
                    input_dim=int(T_dim) * int(embed_size),
                    embed_dim=self.confounder_graph_embed_dim,
                )
    ...
    def _init_env_exogenous_parameters(self):
        for module in [
            ...
            self.load_level_env_embedding,
            self.env_exog_proj,
            self.env_period_proj,
        ]:
    ...
    def _load_pretrained_inv_agcrn(self):
        ...
        for key, value in state.items():
            ...
            elif key in {"end_conv.weight", "module.end_conv.weight"}:
                end_conv_weight = value
            elif key in {"end_conv.bias", "module.end_conv.bias"}:
                end_conv_bias = value
        ...
        head_loaded = False
        with torch.no_grad():
            if end_conv_weight is None or end_conv_bias is None:
                print(
                    "[StableST] WARNING pretrained AGCRN checkpoint has no usable "
                    "end_conv prediction head; loading/frozen encoder only.",
                    flush=True,
                )
            else:
                expected = self.invariant_predict_conv_2.weight
                mapped_weight = end_conv_weight.permute(0, 3, 1, 2).contiguous()
                if mapped_weight.shape == expected.shape:
                    self.invariant_predict_conv_2.weight.copy_(mapped_weight)
                    self.invariant_predict_conv_2.bias.copy_(end_conv_bias)
                    if self.fpem_use_load_level_experts:
                        self.variant_predict_conv_2.weight.copy_(mapped_weight)
                        self.variant_predict_conv_2.bias.copy_(end_conv_bias)
                    head_loaded = True
                else:
                    print(
                        "[StableST] WARNING pretrained AGCRN end_conv head shape "
                        "does not match STEVE invariant head: {} vs {}; "
                        "loading/frozen encoder only and leaving STEVE temporal/"
                        "prediction heads trainable.".format(
                            tuple(mapped_weight.shape), tuple(expected.shape)
                        ),
                        flush=True,
                    )
        self._pretrained_inv_agcrn_prediction_head_loaded = bool(head_loaded)
        print(
            "[StableST] loaded pretrained invariant AGCRN encoder{} "
            "from {}; missing={} unexpected={}".format(
                " and prediction head" if head_loaded else " (prediction head not loaded)",
                path,
    ...
    def freeze_invariant_encoder(self):
        ...
        if getattr(self, "_pretrained_inv_agcrn_prediction_head_loaded", False):
            for param in self.invariant_predict_conv_2.parameters():
                param.requires_grad_(False)
            for param in self.tcl4h.parameters():
                param.requires_grad_(False)
        else:
            print(
                "[StableST] pretrained invariant prediction head was not loaded; "
                "freezing encoder_inv only and keeping invariant prediction heads trainable.",
                flush=True,
            )
    ...
    def set_fpem_epoch(self, epoch):
        self._fpem_current_epoch = max(int(epoch), 0)
        if (
            self.fpem_counterfactual_risk_router
            and self._counterfactual_risk_stage2_active(self._fpem_current_epoch)
        ):
            self._set_counterfactual_risk_stage2_trainable()

    def _counterfactual_risk_stage2_active(self, epoch=None):
        if not self.fpem_counterfactual_risk_router:
            return False
        if epoch is None:
            epoch = self._fpem_current_epoch
        return int(epoch) >= max(int(self.fpem_counterfactual_risk_stage2_start_epoch), 1)

    def _set_counterfactual_risk_stage2_trainable(self):
        for name, param in self.named_parameters():
            param.requires_grad_(name.startswith("hard_environment_use_router."))

    def _cached_observable_load(
        self,
        x,
        ...
        """Validate and detach the precomputed, historical-only cache fields."""
        if (
            observable_load_profile is None
            or observable_load_score is None
            or cached_load_level is None
        ):
            raise ValueError(
                "observable-load prior mode requires cached profile, score, and level"
            )
        profile = observable_load_profile.detach().to(device=x.device)
        score = observable_load_score.detach().to(
            device=x.device, dtype=torch.float32
        ).reshape(-1)
        level = cached_load_level.detach().to(
            device=x.device, dtype=torch.long
        ).reshape(-1)
        if profile.dim() == 3:
            profile = profile.unsqueeze(1)
        expected_profile = (
            x.shape[0],
            1,
            x.shape[2],
            int(getattr(self.args, "d_input", x.shape[-1])),
        )
        if tuple(profile.shape) != expected_profile:
            raise ValueError(
                "cached observable profile shape {} != {}".format(
                    tuple(profile.shape), expected_profile
                )
            )
        if score.shape[0] != x.shape[0] or level.shape[0] != x.shape[0]:
            raise ValueError("cached observable score/level batch size mismatch")
        if bool((profile < 0).any()) or bool((profile > 5).any()):
            raise ValueError("cached observable profile must be in [0,5]")
        if bool((level < 0).any()) or bool(
            (level >= self.fpem_load_level_k).any()
        ):
            raise ValueError("cached observable load level is outside [0,K)")
        return profile, score, level

    def _observable_load_score_and_level(self, x, training=False, sample_index=None):
        """Use only the latest historical traffic observation; never c or y."""
        latest_normalized = x[:, -1].detach().float()
        latest_raw = (
            latest_normalized * float(self.fpem_input_scaler_std)
            + float(self.fpem_input_scaler_mean)
        )
        score = latest_raw.mean(dim=tuple(range(1, latest_raw.dim()))).detach()
        level = assign_load_levels(score, self.load_level_thresholds)
        if self.fpem_randomize_train_load_level and training:
            if sample_index is None:
                raise ValueError("randomized training load levels require sample_index")
            index = sample_index.detach().long().reshape(-1).to(
                device=self.random_train_load_levels.device
            )
            if index.numel() != score.numel():
                raise ValueError("sample_index batch size does not match observed load score")
            if self.random_train_load_levels.numel() == 0:
                raise ValueError("randomized training load levels were not fitted by the data loader")
            if bool((index < 0).any()) or bool((index >= self.random_train_load_levels.numel()).any()):
                raise ValueError("sample_index is outside randomized load assignment buffer")
            level = self.random_train_load_levels.index_select(0, index).to(
                device=x.device
            ).detach()
        return score.to(device=x.device), level.to(device=x.device).detach()

    def get_load_level_state_for_checkpoint(self):
        return {
            "thresholds": self.load_level_thresholds.detach().cpu().tolist(),
            "mode": self.fpem_load_level_mode,
            "k": int(self.fpem_load_level_k),
            "source": (
                "observable_load_prior_k3_v1 cache"
                if self.fpem_use_observable_load_prior
                else "latest historical x[:, -1].mean over nodes and channels"
            ),
            "uses_target_or_future_load": False,
            "randomize_train_only": bool(self.fpem_randomize_train_load_level),
            "uses_cached_observable_prior": bool(
                self.fpem_use_observable_load_prior
            ),
            "cache_path": str(
                getattr(self.args, "fpem_observable_load_prior_cache", "")
            ),
        }

    def load_load_level_state_from_checkpoint(self, state):
        if not state:
            return
        thresholds = torch.as_tensor(
            state.get("thresholds", []),
            device=self.load_level_thresholds.device,
            dtype=self.load_level_thresholds.dtype,
        ).reshape(-1)
        if thresholds.shape != self.load_level_thresholds.shape:
            raise ValueError(
                "checkpoint load thresholds shape mismatch: {} vs {}".format(
                    tuple(thresholds.shape), tuple(self.load_level_thresholds.shape)
                )
            )
        self.load_level_thresholds.copy_(thresholds)
    ...
    def _zero_env_period_logs(self, ref, available=0.0):
        return {
            "fpem/fpem_env_use_period_context": ref.new_tensor(float(self.fpem_env_use_period_context)),
            "fpem/env_period_context_available": ref.new_tensor(float(available)),
            "fpem/env_period_day_available_ratio": ref.new_zeros(()),
            "fpem/env_period_week_available_ratio": ref.new_zeros(()),
            "fpem/env_period_context_feature_dim": ref.new_tensor(
                float(self.fpem_env_period_context_channels * 3)
            ),
            "fpem/env_period_context_scale": ref.new_tensor(float(self.fpem_env_period_context_scale)),
            "fpem/env_period_context_delta_norm": ref.new_zeros(()),
            "fpem/env_period_context_day_diff_norm": ref.new_zeros(()),
            "fpem/env_period_context_week_diff_norm": ref.new_zeros(()),
        }

    def _route_aux_stats_from_inputs(
        self, x, time_label=None, exog=None, observable_load_profile=None
    ):
        ...
        if self.fpem_use_observable_load_prior:
            if observable_load_profile is None:
                raise ValueError(
                    "observable-load route features require cached c_obs"
                )
            c_latest = observable_load_profile[:, -1].detach().float()
        ...
        elif self.fpem_use_load_level_experts:
            latest = (
                x[:, -1].detach().float() * float(self.fpem_input_scaler_std)
                + float(self.fpem_input_scaler_mean)
            )
            latest_flat = latest.reshape(bsz, -1)
            c_stats = torch.stack(
                [
                    latest_flat.mean(dim=1),
                    latest_flat.std(dim=1, unbiased=False),
                    latest_flat.max(dim=1)[0],
                ],
                dim=-1,
            )
        elif exog is not None and torch.is_tensor(exog) and exog.shape[0] == bsz and exog.shape[2] == x.shape[2]:
    ...
    def _load_exogenous_features(
        self, x, exog=None, observable_load_profile=None
    ):
        b, _t, n, _d = x.shape
        if self.fpem_use_observable_load_prior:
            if observable_load_profile is None:
                raise ValueError(
                    "observable-load environment encoder requires cached c_obs"
                )
            c_latest = observable_load_profile[:, -1].detach()
            if (
                c_latest.dim() != 3
                or c_latest.shape[0] != b
                or c_latest.shape[1] != n
            ):
                raise ValueError("cached c_obs has incompatible shape")
            if c_latest.shape[-1] < self.fpem_env_exogenous_load_channels:
                pad = c_latest.new_zeros(
                    b,
                    n,
                    self.fpem_env_exogenous_load_channels
                    - c_latest.shape[-1],
                )
        ...
        if self.fpem_use_load_level_experts:
            level = self._latest_load_level
            if not torch.is_tensor(level) or level.shape[0] != b:
                return None
            load_emb = self.load_level_env_embedding(
                level.detach().to(device=x.device, dtype=torch.long)
            )
            return load_emb[:, None, :].expand(-1, n, -1)
        if self.fpem_ignore_future_c:
            return None
    ...
    def _apply_env_exogenous(
        ...
        load_exog = self._load_exogenous_features(
            x,
            exog=exog,
    ...
    def _fit_period_channels(self, tensor, ref):
        if tensor is None or not torch.is_tensor(tensor):
            return None
        if tensor.shape[:3] != ref.shape[:3]:
            return None
        out = tensor.to(device=ref.device, dtype=ref.dtype)
        c = self.fpem_env_period_context_channels
        if out.shape[-1] < c:
            pad = out.new_zeros(*out.shape[:-1], c - out.shape[-1])
            out = torch.cat([out, pad], dim=-1)
        return out[..., :c]

    def _apply_env_period_context(
        self,
        x_env,
        x_current,
        period_context_day=None,
        period_context_week=None,
        period_context_valid=None,
    ):
        logs = self._zero_env_period_logs(x_env, available=0.0)
        if not self.fpem_env_use_period_context:
            return x_env, logs
        day = self._fit_period_channels(period_context_day, x_current)
        week = self._fit_period_channels(period_context_week, x_current)
        if day is None or week is None:
            if not self._env_period_warned_missing:
                print(
                    "[StableST] WARNING fpem_env_use_period_context=true but day/week "
                    "period context is unavailable or shape-mismatched; using original environment input.",
                    flush=True,
                )
                self._env_period_warned_missing = True
            return x_env, logs
        current = self._fit_period_channels(x_current, x_current)
        features = torch.cat([current, day, week], dim=-1)
        delta = self.env_period_proj(features).to(dtype=x_env.dtype)
        delta = self.fpem_env_period_context_scale * delta
        if period_context_valid is not None and torch.is_tensor(period_context_valid):
            valid = period_context_valid.to(device=x_env.device, dtype=x_env.dtype)
            if valid.dim() == 2 and valid.shape[0] == x_env.shape[0] and valid.shape[1] >= 2:
                day_ratio = valid[:, 0].mean()
                week_ratio = valid[:, 1].mean()
            else:
                day_ratio = x_env.new_zeros(())
                week_ratio = x_env.new_zeros(())
        else:
            day_ratio = x_env.new_tensor(1.0)
            week_ratio = x_env.new_tensor(1.0)
        logs.update({
            "fpem/env_period_context_available": x_env.new_tensor(1.0),
            "fpem/env_period_day_available_ratio": day_ratio,
            "fpem/env_period_week_available_ratio": week_ratio,
            "fpem/env_period_context_delta_norm": delta.detach().float().norm(dim=-1).mean().to(dtype=x_env.dtype),
            "fpem/env_period_context_day_diff_norm": (current - day).detach().float().norm(dim=-1).mean().to(dtype=x_env.dtype),
            "fpem/env_period_context_week_diff_norm": (current - week).detach().float().norm(dim=-1).mean().to(dtype=x_env.dtype),
        })
        return x_env + delta, logs
    ...
    def _confounder_dependence_regularization(self, output, training, epoch):
        ref = output["C_cur"]
        if (
            str(getattr(self, "confounder_dep_mode", "none")).lower() == "none"
            or self.latent_confounder_extractor is None
        ):
            return ref.new_zeros(()), zero_confounder_dependence_logs(ref, mode=self.confounder_dep_mode)
        if not training:
            return ref.new_zeros(()), zero_confounder_dependence_logs(ref, mode=self.confounder_dep_mode)
        return confounder_dependence_terms(
            self.args,
            self.latent_confounder_extractor,
            self.adj,
            output.get("Z_env_seq", None),
            graph_learner=self.latent_confounder_graph_learner,
            graph_target_adj=self._confounder_agcrn_adaptive_adj(ref),
            epoch=epoch,
            training=training,
            ref=ref,
            virtual_queue=self._confounder_virtual_batch_queue,
        )

    def _confounder_agcrn_adaptive_adj(self, ref):
        """Detached pretrained AGCRN adaptive adjacency for graph alignment.

        When the invariant AGCRN is loaded from a pretrained checkpoint, the
        graph teacher must come from that frozen encoder rather than from the
        randomly/actively trained environment encoder.  If no pretrained
        invariant AGCRN is active, keep the historical environment fallback for
        non-pretrained experiments.
        """
        if bool(getattr(self, "fpem_use_pretrained_inv_agcrn", False)):
            encoder = getattr(self, "encoder_inv", None)
        else:
            encoder = getattr(self, "encoder_env", None)
        node_embeddings = getattr(encoder, "node_embeddings", None)
        if node_embeddings is None or not torch.is_tensor(node_embeddings):
            return None
        adj = torch.softmax(
            torch.relu(torch.mm(node_embeddings, node_embeddings.transpose(0, 1))),
            dim=1,
        )
        return adj.detach().to(device=ref.device, dtype=torch.float32)
    ...
    def _observable_route_features(self, z_inv, e_useful, y_heads):
        ...
        if z_float.dim() == 4:
            z_mean = z_float.mean(dim=(1, 2))
            z_std = z_float.reshape(bsz, -1, z_float.shape[-1]).std(dim=1, unbiased=False)
        else:
            z_mean = z_float.mean(dim=1)
            z_std = z_float.std(dim=1, unbiased=False)
        if e_float.dim() == 4:
            e_mean = e_float.mean(dim=(1, 2))
            e_std = e_float.reshape(bsz, -1, e_float.shape[-1]).std(dim=1, unbiased=False)
        else:
    ...
    def forward_features(
        ...
        if self.fpem_use_observable_load_prior:
            (
                ...
                observed_score,
                load_level,
            ) = self._cached_observable_load(
                x,
                observable_load_profile,
                observable_load_score,
                cached_load_level,
            )
            self._latest_observable_load_profile = observable_load_profile
            self._latest_observed_load_score = observed_score
            self._latest_load_level = load_level
        elif self.fpem_use_load_level_experts:
            observed_score, load_level = self._observable_load_score_and_level(
                x,
                training=training,
            ...
            self._latest_observed_load_score = None
            self._latest_load_level = None
        self._latest_route_aux_stats = self._route_aux_stats_from_inputs(
            x,
            time_label=time_label,
            exog=None if self.fpem_use_observable_load_prior else exog,
            observable_load_profile=observable_load_profile,
        )
        if self.fpem_use_pretrained_inv_agcrn:
            with torch.no_grad():
        ...
        x_env, env_exog_logs = self._apply_env_exogenous(
            x,
            time_label=time_label,
            exog=None if self.fpem_use_observable_load_prior else exog,
            observable_load_profile=observable_load_profile,
        ...
        x_env, env_period_logs = self._apply_env_period_context(
            x_env,
            x,
            period_context_day=period_context_day,
            period_context_week=period_context_week,
            period_context_valid=period_context_valid,
        )
        self._latest_env_period_logs = env_period_logs
    ...
    def forward(
        ...
        z_inv_seq, z_env_seq = self.forward_features(
            x,
            time_label=time_label,
            exog=c,
            training=bool(training_loss),
            sample_index=sample_index,
            observable_load_profile=observable_load_profile,
    ...
    def _temporal_pool_inv(self, z_inv_seq):
        ...
        if (
            self.fpem_use_pretrained_inv_agcrn
            and getattr(self, "_pretrained_inv_agcrn_prediction_head_loaded", False)
        ):
            # The pretrained AGCRN forecast head consumes the final recurrent
            # state.  Together with the mapped end_conv weights this exactly
            # reconstructs the complete invariant prediction path.
            return z_inv_seq[:, -1]
    ...
    def _configure_trainable_for_selected_mode(self):
        ...
        if not self.fpem_env_use_exogenous:
            ...
            self._set_requires_grad(self.load_level_env_embedding, False)
            self._set_requires_grad(self.env_exog_proj, False)
        if not self.fpem_env_use_period_context:
            self._set_requires_grad(self.env_period_proj, False)
        if self.fpem_use_load_level_experts:
            ...
            if not self.fpem_use_observable_load_prior:
                self._set_requires_grad(self.env_load_embedding, False)
            if not self.fpem_use_environment_gate:
                self._set_requires_grad(self.environment_use_gate, False)
        else:
            self.load_level_env_residual_scale.requires_grad_(False)
            self._set_requires_grad(self.environment_use_gate, False)
            self._set_requires_grad(self.load_level_env_embedding, False)
        if self.fpem_use_observable_load_prior:
            self.load_level_env_residual_scale.requires_grad_(False)
        ...
        if not (
            ...
            self._set_requires_grad(self.hard_environment_use_router, False)
        if self.fpem_force_uniform_route:
            self.env_prototypes.requires_grad_(False)
            self._set_requires_grad(self.hyper_router, False)
    ...
    def _predict_load_level_experts(
        ...
        if self.fpem_use_observable_load_prior:
            return self._predict_observable_load_level_experts(
                output,
                z_inv,
                e_useful,
                y_inv,
        ...
        load_level = self._latest_load_level
        load_score = self._latest_observed_load_score
        if not torch.is_tensor(load_level) or load_level.shape[0] != y_env_heads.shape[0]:
            raise RuntimeError("load-level assignment is unavailable for the current batch")
        load_level = load_level.detach().to(device=y_env_heads.device, dtype=torch.long)
        y_env_selected = select_load_expert(y_env_heads, load_level)
        gate_out = self.environment_use_gate(z_inv, e_useful, load_level)
        predicted_gate = gate_out["env_use_gate"]
        effective_epoch = self._fpem_current_epoch if epoch is None else int(epoch)
        warmup_active = effective_epoch <= max(self.fpem_environment_gate_warmup_epochs, 0)
        if self.fpem_use_environment_gate and not warmup_active:
            env_use_gate = predicted_gate
        else:
            env_use_gate = torch.ones_like(predicted_gate)
        gate_view = env_use_gate.view(-1, 1, 1, 1)
        y_final = (1.0 - gate_view) * y_inv + gate_view * y_env_selected
        route_q = F.one_hot(
            load_level,
            num_classes=self.fpem_env_route_k,
        ).to(device=y_env_heads.device, dtype=y_env_heads.dtype)
        route_logits = torch.log(route_q.float().clamp_min(1e-8)).to(dtype=y_env_heads.dtype)
        route_features = self._observable_route_features(z_inv, e_useful, y_env_heads)
        fusion_logs = self.fusion.zero_logs(y_inv)
        output.update({
            "prediction": y_final,
            "y_final": y_final,
            "y_route": y_env_selected,
            "y_route_heads": y_env_heads,
            "y_hyper_heads": y_env_heads,
            "y_env_heads": y_env_heads,
            "y_env_selected": y_env_selected,
            "y_candidates": y_env_heads,
            "route_q": route_q,
            "env_route_q": route_q,
            "env_route_q_prediction": route_q,
            "env_route_logits": route_logits,
            "env_route_logits_prediction": route_logits,
            "env_route_selected_head": load_level,
            "env_route_entropy_per_sample": route_q.new_zeros(route_q.shape[0]),
            "env_route_q_max": route_q.new_ones(route_q.shape[0]),
            "fallback_q": 1.0 - env_use_gate,
            "load_level": load_level,
            "observable_load_score": load_score,
            "load_level_thresholds": self.load_level_thresholds,
            "env_use_gate": env_use_gate,
            "env_use_gate_predicted": predicted_gate,
            "env_use_gate_logits": gate_out["env_use_gate_logits"],
            "env_use_gate_features": gate_out["env_use_gate_features"],
            "load_level_embedding": gate_out["load_level_embedding"],
            "environment_gate_warmup_active": y_inv.new_tensor(float(warmup_active)),
            "environment_gate_uses_target_in_forward": y_inv.new_zeros(()),
            "fusion_alpha": gate_view,
            ...
            "env_route_target_mode": "observable_load_level",
            "primary_uses_route": y_inv.new_tensor(1.0),
            "primary_uses_env_fusion": y_inv.new_tensor(float(self.fpem_use_environment_gate)),
            "training": bool(training),
            "fusion_logs": fusion_logs,
        })
        return output

    def _predict_observable_load_level_experts(
        self,
        output,
        z_inv,
        e_useful,
        ...
        """Fixed observable expert plus one hard invariant/environment router."""
        load_level = self._latest_load_level
        load_score = self._latest_observed_load_score
        if (
            not torch.is_tensor(load_level)
            or load_level.shape[0] != y_env_heads.shape[0]
        ):
            raise RuntimeError(
                "cached load-level assignment is unavailable for this batch"
            )
        load_level = load_level.detach().to(
            device=y_env_heads.device, dtype=torch.long
        )
        y_env_selected = select_load_expert(y_env_heads, load_level)
        detach_router_features = (
            self.fpem_conservative_inv_override
            or self.fpem_counterfactual_risk_router
        )
        router_z_inv = z_inv.detach() if detach_router_features else z_inv
        router_e_useful = e_useful.detach() if detach_router_features else e_useful
        router_out = self.hard_environment_use_router(
            router_z_inv, router_e_useful, load_level
        )
        argmax_route_id = router_out[
            "hard_router_route_id"
        ].detach().long()
        hard_router_logits = router_out["hard_router_logits"]
        effective_epoch = (
            self._fpem_current_epoch if epoch is None else int(epoch)
        )
        conservative_inv_logit = hard_router_logits[:, 0] - hard_router_logits[:, 1]
        conservative_p_inv = torch.sigmoid(conservative_inv_logit)
        conservative_use_inv = (
            conservative_p_inv > float(self.fpem_override_threshold)
        )
        conservative_route_id = torch.where(
            conservative_use_inv,
            torch.zeros_like(argmax_route_id),
            torch.ones_like(argmax_route_id),
        ).detach().long()
        counterfactual_predicted_risks = hard_router_logits.float()
        counterfactual_risk_argmin = counterfactual_predicted_risks.detach().argmin(dim=-1)
        # predicted risk order is [env, inv], while hard_select uses 0=inv, 1=env.
        counterfactual_risk_route_id = torch.where(
            counterfactual_risk_argmin == 1,
            torch.zeros_like(argmax_route_id),
            torch.ones_like(argmax_route_id),
        ).detach().long()
        counterfactual_risk_stage2_active = self._counterfactual_risk_stage2_active(
            effective_epoch
        )
        predicted_route_id = (
            counterfactual_risk_route_id
            if self.fpem_counterfactual_risk_router
            else conservative_route_id
            if self.fpem_conservative_inv_override
            else argmax_route_id
        )
        warmup_active = bool(
            training
            and effective_epoch
            <= max(self.fpem_hard_router_warmup_epochs, 0)
        )
        # Counterfactual-risk router training is target-supervised risk
        # regression/ranking only: during train() we keep the top-level
        # prediction on the default environment path and do not actually route
        # samples.  The learned argmin hard route is used only for eval/test
        # deployment/audit.
        counterfactual_risk_deploy_route_active = (
            self.fpem_counterfactual_risk_router
            and counterfactual_risk_stage2_active
            and not bool(training)
        )
        if (
            self.fpem_use_hard_environment_router
            or self.fpem_conservative_inv_override
            or counterfactual_risk_deploy_route_active
        ) and not warmup_active:
            hard_route_id = predicted_route_id
        else:
            hard_route_id = torch.ones_like(predicted_route_id)
        y_final = hard_select_invariant_or_environment(
            y_inv, y_env_selected, hard_route_id
        )
        route_q = F.one_hot(
            load_level, num_classes=self.fpem_env_route_k
        ).to(device=y_env_heads.device, dtype=y_env_heads.dtype)
        route_logits = torch.log(route_q.float().clamp_min(1e-8)).to(
            dtype=y_env_heads.dtype
        )
        hard_route_mask = hard_route_id.to(dtype=y_inv.dtype).view(
            -1, 1, 1, 1
        )
        route_features = self._observable_route_features(
            z_inv, e_useful, y_env_heads
        )
        fusion_logs = self.fusion.zero_logs(y_inv)
        output.update(
            {
                ...
                "env_route_logits_prediction": route_logits,
                "env_route_selected_head": load_level,
                "env_route_entropy_per_sample": route_q.new_zeros(
                    route_q.shape[0]
                ),
                "env_route_q_max": route_q.new_ones(route_q.shape[0]),
                "fallback_q": (hard_route_id == 0).to(dtype=y_inv.dtype),
                "load_level": load_level,
                "observable_load_score": load_score,
                "observable_load_profile": self._latest_observable_load_profile,
                "load_level_thresholds": self.load_level_thresholds,
                "hard_router_logits": router_out["hard_router_logits"],
                "hard_router_probabilities": router_out[
                    "hard_router_probabilities"
                ],
                "hard_router_predicted_route_id": predicted_route_id,
                "hard_router_argmax_route_id": argmax_route_id,
                "hard_route_id": hard_route_id.detach(),
                "route_id": hard_route_id.detach(),
                "conservative_inv_logit": conservative_inv_logit,
                "conservative_p_inv": conservative_p_inv,
                "conservative_use_inv": conservative_use_inv.detach().to(
                    dtype=y_inv.dtype
                ),
                "conservative_override_threshold": y_inv.new_tensor(
                    float(self.fpem_override_threshold)
                ),
                "conservative_override_margin": y_inv.new_tensor(
                    float(self.fpem_override_margin)
                ),
                "conservative_override_enabled": y_inv.new_tensor(
                    float(self.fpem_conservative_inv_override)
                ),
                "conservative_router_features_detached": y_inv.new_tensor(
                    float(detach_router_features)
                ),
                "counterfactual_predicted_risks": counterfactual_predicted_risks,
                "counterfactual_risk_argmin": counterfactual_risk_argmin.detach(),
                "counterfactual_risk_route_id": counterfactual_risk_route_id.detach(),
                "counterfactual_risk_stage2_active": y_inv.new_tensor(
                    float(counterfactual_risk_stage2_active)
                ),
                "counterfactual_risk_deploy_route_active": y_inv.new_tensor(
                    float(counterfactual_risk_deploy_route_active)
                ),
                "counterfactual_risk_training_uses_route": y_inv.new_tensor(
                    float(
                        self.fpem_counterfactual_risk_router
                        and bool(training)
                        and counterfactual_risk_deploy_route_active
                    )
                ),
                "counterfactual_risk_router_enabled": y_inv.new_tensor(
                    float(self.fpem_counterfactual_risk_router)
                ),
                "counterfactual_risk_uses_threshold": y_inv.new_zeros(()),
                "hard_router_features": router_out[
                    "hard_router_features"
                ],
                "load_level_embedding": router_out[
                    "load_level_embedding"
                ],
                "hard_router_warmup_active": y_inv.new_tensor(
                    float(warmup_active)
                ),
                "hard_router_uses_target_in_forward": y_inv.new_zeros(()),
                # Compatibility diagnostics; this is a binary mask and is
                # never used to blend complete predictions.
                "env_use_gate": hard_route_id.to(dtype=y_inv.dtype),
                "env_use_gate_predicted": predicted_route_id.to(
                    dtype=y_inv.dtype
                ),
                "env_use_gate_logits": (
                    router_out["hard_router_logits"][:, 1]
                    - router_out["hard_router_logits"][:, 0]
                ),
                "environment_gate_warmup_active": y_inv.new_tensor(
                    float(warmup_active)
                ),
                "environment_gate_uses_target_in_forward": y_inv.new_zeros(
                    ()
                ),
                "fusion_alpha": hard_route_mask,
                "hard_route_mask": hard_route_mask,
                "hyper_alpha": hyper_out["hyper_alpha"],
                "hyper_delta_norm": hyper_out["hyper_delta_norm"],
                "hyper_gamma_norm_per_head": hyper_out[
                    "hyper_gamma_norm_per_head"
                ],
                "hyper_beta_norm_per_head": hyper_out[
                    "hyper_beta_norm_per_head"
                ],
                "route_features": route_features,
                "route_head_mode": self.fpem_env_route_head_mode,
                "env_route_target_mode": "cached_observable_load_level",
                "primary_uses_route": y_inv.new_tensor(1.0),
                "primary_uses_env_fusion": y_inv.new_zeros(()),
                "hard_binary_router_enabled": y_inv.new_tensor(
                    float(
                        self.fpem_use_hard_environment_router
                        or self.fpem_conservative_inv_override
                    )
                ),
                "conservative_inv_override_enabled": y_inv.new_tensor(
                    float(self.fpem_conservative_inv_override)
                ),
                "counterfactual_risk_router_enabled": y_inv.new_tensor(
                    float(self.fpem_counterfactual_risk_router)
    ...
    def _load_level_expert_parameter_norm_logs(self, ref):
        logs = {}
        heads = self.hyper_concat_input_heads if self._use_hyper_input_concat() else self.hyper_inv_heads
        for idx in range(self.fpem_env_route_k):
            squares = []
            for module in (heads.hypernets[idx], heads.alpha_gates[idx]):
                for param in module.parameters():
                    squares.append(param.detach().float().pow(2).sum())
            norm = torch.stack(squares).sum().sqrt().to(device=ref.device, dtype=ref.dtype)
            logs[f"fpem/expert_{idx}_parameter_norm"] = norm
        return logs

    @staticmethod
    def _load_level_pairwise_logs(output):
        y_heads = output["y_env_heads"]
        y_inv = output["y_inv"]
        zero = y_inv.new_zeros(())
        if y_heads.shape[1] <= 1:
            return {
                "fpem/expert_prediction_pairwise_difference": zero,
                "fpem/expert_prediction_pairwise_cosine": zero,
                "fpem/delta_pairwise_cosine": zero,
                "fpem/delta_pairwise_l1": zero,
                "fpem/delta_pairwise_l2": zero,
            }
        pred_flat = y_heads.detach().float().permute(1, 0, 2, 3, 4).reshape(y_heads.shape[1], -1)
        delta = (y_heads.detach() - y_inv.detach().unsqueeze(1)).float()
        delta_flat = delta.permute(1, 0, 2, 3, 4).reshape(y_heads.shape[1], -1)
        pred_cosines, delta_cosines, pred_l1, delta_l1, delta_l2 = [], [], [], [], []
        for left, right in itertools.combinations(range(y_heads.shape[1]), 2):
            pred_cosines.append(F.cosine_similarity(pred_flat[left], pred_flat[right], dim=0))
            delta_cosines.append(F.cosine_similarity(delta_flat[left], delta_flat[right], dim=0))
            pred_diff = pred_flat[left] - pred_flat[right]
            delta_diff = delta_flat[left] - delta_flat[right]
            pred_l1.append(pred_diff.abs().mean())
            delta_l1.append(delta_diff.abs().mean())
            delta_l2.append(delta_diff.pow(2).mean().sqrt())
        return {
            "fpem/expert_prediction_pairwise_difference": torch.stack(pred_l1).mean().to(dtype=y_inv.dtype),
            "fpem/expert_prediction_pairwise_cosine": torch.stack(pred_cosines).mean().to(dtype=y_inv.dtype),
            "fpem/delta_pairwise_cosine": torch.stack(delta_cosines).mean().to(dtype=y_inv.dtype),
            "fpem/delta_pairwise_l1": torch.stack(delta_l1).mean().to(dtype=y_inv.dtype),
            "fpem/delta_pairwise_l2": torch.stack(delta_l2).mean().to(dtype=y_inv.dtype),
        }
        ...
        if hyper_mode:
            ...
            if self._use_hyper_input_concat():
                ...
                if (
                    ...
                    hyper_h = z_inv + self.load_level_env_residual_scale * e_useful
                else:
                    hyper_h = z_inv + e_useful
            ...
            if self.fpem_use_load_level_experts:
                return self._predict_load_level_experts(
                    output,
                    z_inv,
                    e_useful,
                    y_inv,
    ...
    def forward_output_from_features(self, z_inv_seq, z_env_seq, exog=None, training=False, epoch=None, sample_index=None):
        ...
        output.update({
            ...
            "env_period_logs": getattr(self, "_latest_env_period_logs", self._zero_env_period_logs(z_env_seq)),
    ...
    def calculate_loss(
        ...
        counterfactual_risk_stage2_active = (
            self.fpem_counterfactual_risk_router
            and self._counterfactual_risk_stage2_active(epoch)
        )
        ...
        if training and counterfactual_risk_stage2_active:
            loss = primary_loss.new_zeros(())
        elif training and hard_sinkhorn_family_mode:
        ...
        if training:
            ...
            if (
                counterfactual_risk_stage2_active
                or (self._use_hyper_proto_film() and not self.fpem_env_route_use_inv_fallback_expert)
                or hard_sinkhorn_family_mode
            ):
                inv_lambda = 0.0
        ...
        if self.fpem_use_env_route and self._route_enabled_for_epoch(training, epoch):
            if self.fpem_use_load_level_experts:
                if self.fpem_use_observable_load_prior:
                    if self.fpem_counterfactual_risk_router:
                        (
                            load_expert_loss,
                            environment_gate_loss,
                            load_logs,
                            load_gate_diagnostics,
                        ) = load_level_expert_counterfactual_risk_router_losses(
                            output,
                            target,
                            scaler,
                        ...
                        if counterfactual_risk_stage2_active:
                            route_loss = (
                                self.fpem_counterfactual_risk_router_loss_weight
                                * environment_gate_loss
                            )
                        else:
                            route_loss = self.fpem_lambda_load_expert * load_expert_loss
                    elif self.fpem_conservative_inv_override:
                        (
                            load_expert_loss,
                            environment_gate_loss,
                            load_logs,
                        ...
                        route_loss = (
                            self.fpem_lambda_load_expert * load_expert_loss
                            + self.fpem_override_router_loss_weight
                            * environment_gate_loss
                        )
                    else:
                        (
                    ...
                    route_loss = (
                        self.fpem_lambda_load_expert * load_expert_loss
                        + self.fpem_lambda_environment_gate
                        * environment_gate_loss
                    )
                route_logs.update(load_logs)
                route_logs.update(self._load_level_pairwise_logs(output))
                route_logs.update(self._load_level_expert_parameter_norm_logs(primary_loss))
                route_logs.update({
                    "fpem/env_route_loss": route_loss.detach(),
                    "fpem/env_route_L_final": primary_loss.detach(),
                    "fpem/env_route_L_expert": load_expert_loss.detach(),
                    "fpem/env_route_L_router_oracle": environment_gate_loss.detach(),
                    "fpem/fallback_q_mean": (1.0 - output["env_use_gate"].detach()).mean(),
                    "fpem/env_q_sum_mean": output["env_use_gate"].detach().mean(),
                    "fpem/effective_expert_number": primary_loss.new_tensor(
                        float(torch.unique(output["load_level"].detach()).numel())
                    ),
                })
                if self.fpem_use_observable_load_prior:
                    route_logs.update(
                        {
                            "fpem/hard_binary_router_enabled": output[
                                "hard_binary_router_enabled"
                            ].detach(),
                            "fpem/hard_router_uses_target_in_forward": output[
                                "hard_router_uses_target_in_forward"
                            ].detach(),
                            "fpem/stored_future_c_loaded": primary_loss.new_zeros(
                                ()
                            ),
                            "fpem/prediction_is_weighted_complete_fusion": primary_loss.new_zeros(
                                ()
                            ),
                            "fpem/prediction_is_hard_route": primary_loss.new_tensor(
                                1.0
                            ),
                            "fpem/conservative_inv_override_enabled": output[
                                "conservative_inv_override_enabled"
                            ].detach(),
                            "fpem/conservative_router_features_detached": output[
                                "conservative_router_features_detached"
                            ].detach(),
                            "fpem/counterfactual_risk_router_enabled": output[
                                "counterfactual_risk_router_enabled"
                            ].detach(),
                            "fpem/counterfactual_risk_stage2_active_forward": output[
                                "counterfactual_risk_stage2_active"
                            ].detach(),
                            "fpem/counterfactual_risk_deploy_route_active": output[
                                "counterfactual_risk_deploy_route_active"
                            ].detach(),
                            "fpem/counterfactual_risk_training_uses_route": output[
                                "counterfactual_risk_training_uses_route"
                            ].detach(),
                            "fpem/counterfactual_risk_forward_uses_threshold": output[
                                "counterfactual_risk_uses_threshold"
                            ].detach(),
                        }
                    )
                q_route_diag = output["env_route_q"].detach()
                level = output["load_level"].detach()
                loss_head_diag = load_gate_diagnostics["loss_head"]
                for idx in range(self.fpem_env_route_k):
                    mask = level == idx
                    count = mask.sum().to(dtype=primary_loss.dtype)
                    ratio = count / max(float(level.numel()), 1.0)
                    route_logs[f"fpem/env_route_count_head_{idx}"] = count
                    route_logs[f"fpem/env_route_oracle_count_head_{idx}"] = count
                    route_logs[f"fpem/route_count_env_head_{idx}"] = count
                    route_logs[f"fpem/route_soft_mean_expert_{idx}"] = ratio
                    route_logs[f"fpem/route_hard_count_expert_{idx}"] = count
                    route_logs[f"fpem/hard_count_expert_{idx}"] = count
                    for expert_idx in range(self.fpem_env_route_k):
                        route_logs[f"fpem/expert_cross_mae_group_{idx}_expert_{expert_idx}"] = (
                            loss_head_diag[mask, expert_idx].mean()
                            if bool(mask.any())
                            else primary_loss.new_zeros(())
                        )
            elif warmup_risk_sinkhorn_mode:
            ...
            if training:
                ...
                if self.fpem_use_load_level_experts:
                    self._expert_zero_streak = 0
                elif hard_counts and min(hard_counts) <= 0.0:
        ...
        if training and self.fpem_use_future_mi and not counterfactual_risk_stage2_active:
            if self.fpem_use_confounder_extractor:
        ...
        if training and not counterfactual_risk_stage2_active:
        ...
        conf_dep_loss, conf_dep_logs = self._confounder_dependence_regularization(
            output,
            training and not counterfactual_risk_stage2_active,
            epoch,
        )
        if training and not counterfactual_risk_stage2_active:
            loss = loss + env_cls_loss + env_supcon_loss + inv_adv_loss + xcov_loss + conf_dep_loss

        logs = {}
        logs.update(output["env_exog_logs"])
        logs.update(output["env_period_logs"])
        ...
        logs.update(conf_dep_logs)
        logs.update(club_fit_logs)
        logs.update(club_upper_logs)
        if self.fpem_use_load_level_experts:
            logs["fpem/load_level_experts_enabled"] = primary_loss.new_tensor(1.0)
            logs["fpem/load_level_uses_target_or_future_load"] = primary_loss.new_zeros(())
            logs["fpem/environment_gate_uses_target_in_forward"] = output[
                "environment_gate_uses_target_in_forward"
            ]
            logs["fpem/load_level_env_residual_scale"] = (
                self.load_level_env_residual_scale.detach()
            )
            logs["fpem/uses_cached_observable_load_prior"] = (
                primary_loss.new_tensor(
                    float(self.fpem_use_observable_load_prior)
                )
            )
            if self.fpem_use_observable_load_prior:
                logs["fpem/stored_future_c_loaded"] = (
                    primary_loss.new_zeros(())
                )
        ...
        self.latest_fpem_outputs = {
            ...
            "load_level": output.get("load_level"),
            "observable_load_score": output.get("observable_load_score"),
            "observable_load_profile": output.get(
                "observable_load_profile"
            ),
            "y_env_heads": output.get("y_env_heads"),
            "y_env_selected": output.get("y_env_selected"),
            "env_use_gate": output.get("env_use_gate"),
            "env_use_gate_logits": output.get("env_use_gate_logits"),
            "y_final": output.get("y_final"),
            "gate_target": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get(
                    "gate_target",
                    load_gate_diagnostics.get("router_target"),
                )
            ),
            "gate_gain": None if load_gate_diagnostics is None else load_gate_diagnostics.get("gain"),
            "router_target": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get("router_target")
            ),
            "hard_router_logits": output.get("hard_router_logits"),
            "hard_router_probabilities": output.get(
                "hard_router_probabilities"
            ),
            "hard_router_predicted_route_id": output.get(
                "hard_router_predicted_route_id"
            ),
            "hard_router_argmax_route_id": output.get("hard_router_argmax_route_id"),
            "conservative_inv_logit": output.get("conservative_inv_logit"),
            "conservative_p_inv": output.get("conservative_p_inv"),
            "conservative_use_inv": output.get("conservative_use_inv"),
            "conservative_delta": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get("delta")
            ),
            "conservative_threshold": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get("threshold")
            ),
            "conservative_margin": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get("margin")
            ),
            "counterfactual_predicted_risks": output.get("counterfactual_predicted_risks"),
            "counterfactual_risk_argmin": output.get("counterfactual_risk_argmin"),
            "counterfactual_risk_route_id": output.get("counterfactual_risk_route_id"),
            "counterfactual_risk_stage2_active": output.get("counterfactual_risk_stage2_active"),
            "counterfactual_risk_target": (
                None
                if load_gate_diagnostics is None
                else load_gate_diagnostics.get("risk_target")
            ),
            "hard_route_id": output.get("hard_route_id"),
