...
class EnvConditionedInvariantHeads(nn.Module):
    ...
    @staticmethod
    def _pool_context(feat):
        if feat.dim() == 3:
            return feat.mean(dim=1)
        if feat.dim() == 4:
            return feat.mean(dim=(1, 2))
        raise ValueError(
            "FiLM context feature must be [B,N,H] or [B,T,N,H], got {}".format(
                tuple(feat.shape)
            )
        )

    def forward(self, z_inv, e_useful, h_inv, inv_pred):
        pool_z = self._pool_context(z_inv)
        pool_e = self._pool_context(e_useful)
        ...
        for hypernet, alpha_gate in zip(self.hypernets, self.alpha_gates):
            ...
            gamma_node = gamma.view(gamma.shape[0], *([1] * (h_inv.dim() - 2)), gamma.shape[-1])
            beta_node = beta.view(beta.shape[0], *([1] * (h_inv.dim() - 2)), beta.shape[-1])
            alpha_node = alpha.view(alpha.shape[0], *([1] * (h_inv.dim() - 1)))
