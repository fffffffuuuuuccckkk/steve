...
import numpy as np
from models.our_model import StableST as FPEMStableST
from models.steve_original import OriginalStableST


def _str2bool_local(value):
...
def _has_fpem_config(args):
    return any(key.startswith('fpem_') for key in vars(args).keys())


def _model_impl(args):
    impl = getattr(args, 'model_impl', None)
    if impl is None or str(impl).strip() == '':
        impl = 'fpem' if _has_fpem_config(args) else 'steve_original'
    impl = str(impl).strip().lower()
    if impl in ('original', 'original_steve', 'steve', 'steve_original'):
        return 'steve_original'
    if impl in ('fpem', 'fpem_steve'):
        return 'fpem'
    raise ValueError("model_impl must be one of: steve_original, fpem")


def _build_model(args, A):
    cls = OriginalStableST if _model_impl(args) == 'steve_original' else FPEMStableST
    return cls(args=args, adj=A, in_channels=args.d_input, embed_size=args.d_model,
               T_dim=args.input_length, output_T_dim=1, output_dim=args.d_output,
...
class Trainer(object):
    ...
    def _try_resume(self):
        ...
        if 'optimizer' in state_dict and state_dict['optimizer'] is not None:
            try:
                ...
                self.logger.info('Resume skipped incompatible optimizer state: {}'.format(exc))
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target, time_label,c) in enumerate(self.train_loader):
            ...
            H_inv, Z_var = self.model(data)  # H_inv: invariant, Z_var: variant/environment
            loss, sep_loss,lm = self.model.calculate_loss(H_inv, Z_var, target, c, time_label, self.scaler, loss_weights,p,True)
            if type(lm) == int:
                lms.append(lm)
            else:
                lms.append(lm.item())
            ...
            # t4=datetime.now()
            self.optimizer.step()
            if hasattr(self.model, "clear_fpem_runtime_cache"):
                self.model.clear_fpem_runtime_cache()
            else:
                if hasattr(self.model, "latest_fpem_outputs"):
                    self.model.latest_fpem_outputs = {}
                if hasattr(self.model, "_fpem_gc_primary_grads"):
                    self.model._fpem_gc_primary_grads = {}
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        with torch.no_grad():
            for batch_idx, (data, target,time_label,c) in enumerate(val_dataloader):
                H_inv, Z_var = self.model(data)
                # c_hat=self.model.predict_con(data)
                loss, sep_loss,lm = self.model.calculate_loss(H_inv, Z_var, target, c, time_label, self.scaler, loss_weights)
    ...
    def test(model, dataloader, scaler, graph, logger, args):
        ...
        y_pred = []
        y_true = []
        save_representation = _str2bool_local(getattr(args, "save_representation", False))
        save_samplewise = _str2bool_local(getattr(args, "save_samplewise", False))
        is_original_steve = _model_impl(args) == 'steve_original'
        x = [] if save_representation else None
        atts = [] if save_representation else None
        Cs = [] if save_representation else None
        Hs = [] if save_representation else None
        sample_indices = []
        global_offset = 0
        start_time=time.time()
        with torch.no_grad():
            for batch_idx, (data, target, c) in enumerate(dataloader):
                ...
                # if batch_idx!=10:
                #     continue
                if is_original_steve:
                    H_inv, Z_var = model(data, graph)
                    pred_output, att, C_tensor, H_for_save = model.predict_test(H_inv, Z_var)
                else:
                    output = model.forward_output(data, exog=c, training=False)
                    pred_output = output["prediction"]
                    att = output["env_route_q"]
                    C_tensor = output["E_useful"]
                    H_for_save = output["Z_inv"]
                pred_output = pred_output.squeeze(1)
                target = target.squeeze(1)
                y_true.append(target)
                y_pred.append(pred_output)
                batch_size = int(pred_output.shape[0])
                sample_indices.append(np.arange(global_offset, global_offset + batch_size, dtype=np.int64))
                global_offset += batch_size
                if save_representation:
                    x.append(data.cpu().detach())
                    atts.append(att.cpu().detach())
                    Cs.append(C_tensor.cpu().detach())
                    Hs.append(H_for_save.cpu().detach())
        
        y_true = scaler.inverse_transform(torch.cat(y_true, dim=0))
        y_pred = scaler.inverse_transform(torch.cat(y_pred, dim=0))
        sample_index = np.concatenate(sample_indices, axis=0) if sample_indices else np.zeros((0,), dtype=np.int64)
        ...
        if save_representation:
            ...
            np.savez(rep_path,C=Cs.numpy(),H=Hs.numpy())
        else:
        ...
        if save_samplewise:
            ...
            if pred_np.shape != true_np.shape:
                raise AssertionError(
                    "prediction and target shapes differ: {} vs {}".format(pred_np.shape, true_np.shape)
                )
            if pred_np.shape[0] != sample_index.shape[0]:
                raise AssertionError(
                    "sample_mae length would not match prediction samples: {} vs {}".format(
                        sample_index.shape[0], pred_np.shape[0]
                    )
                )
            sample_abs = np.abs(pred_np - true_np)
            sample_sq = np.square(pred_np - true_np)
            sample_axes = tuple(range(1, pred_np.ndim))
            sample_mae = sample_abs.mean(axis=sample_axes)
            sample_mse = sample_sq.mean(axis=sample_axes)
            if sample_mae.shape[0] != pred_np.shape[0]:
                raise AssertionError("sample_mae.shape[0] must match prediction.shape[0]")
            if len(np.unique(sample_index)) != len(sample_index):
                raise AssertionError("sample_index must be unique for aligned sample-wise comparison")

            np.savez(
                os.path.join(args.log_dir, 'sample_predictions.npz'),
                prediction=pred_np,
                target=true_np,
            ...
            with open(os.path.join(args.log_dir, 'sample_metrics.csv'), 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['sample_index', 'sample_mae', 'sample_mse'])
                for idx, mae_value, mse_value in zip(sample_index, sample_mae, sample_mse):
                    writer.writerow([int(idx), float(mae_value), float(mse_value)])
...
def main(args):
    ...
    model = _build_model(args, A)

    

    if _model_impl(args) == 'steve_original':
        # Keep the original STEVE optimizer protocol: mi_net remains part of the
        # main optimizer, matching the repository's original training entry.
        if hasattr(model, 'optimizer_parameters'):
            main_params = model.optimizer_parameters()
        else:
            main_params = model.parameters()
    else:
        # FPEM keeps mi_net on its own optimizer to avoid double management.
        main_params = [
            param for name, param in model.named_parameters()
            if not name.startswith("mi_net.")
        ]
