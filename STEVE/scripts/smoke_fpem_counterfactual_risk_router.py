...
def main():
    ...
    model.train()
    with torch.no_grad():
        train_output = model.forward_output(
            unpacked["data"],
        ...
        training_route_default_env = bool(
            (train_output["hard_route_id"].detach().long() == 1).all().item()
        )
        training_uses_route = bool(
            float(train_output["counterfactual_risk_training_uses_route"].item()) > 0.5
        )
        stage2_forward_active = bool(
            float(train_output["counterfactual_risk_stage2_active"].item()) > 0.5
        )
    ...
    with torch.no_grad():
        ...
        deploy_route_active = bool(
            float(output["counterfactual_risk_deploy_route_active"].item()) > 0.5
        )
        eval_argmin_route_exact = bool(
            torch.equal(
                output["hard_route_id"].detach().long(),
                output["counterfactual_risk_route_id"].detach().long(),
            )
        )
    ...
    result = {
        ...
        "stage2_forward_active": stage2_forward_active,
        "training_route_default_env": training_route_default_env,
        "training_uses_route": training_uses_route,
        "eval_deploy_route_active": deploy_route_active,
        "eval_argmin_route_exact": eval_argmin_route_exact,
    ...
    failed = [
        key for key in [
            ...
            "stage2_forward_active",
            "training_route_default_env",
            "eval_deploy_route_active",
            "eval_argmin_route_exact",
            "hard_route_exact",
    ...
    if result["training_uses_route"]:
        failed.append("training_must_not_use_route")
