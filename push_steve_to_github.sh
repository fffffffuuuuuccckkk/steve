...
SOURCE_DIR="${SOURCE_DIR:-/data/OuXiaoyu/mystg/baselines/STEVE_CODE}"
...
main() {
  ...
  if ! repo_exists; then
    cat >&2 <<EOF
[push-steve] GitHub repo does not exist and this machine has no repo-creation credential.

Fix one of these, then rerun:
  1. Create an empty repo named '${REPO_NAME}' under https://github.com/${OWNER}
  2. Or export a token with repo scope:
       export GITHUB_TOKEN=ghp_xxx
       OWNER=${OWNER} REPO_NAME=${REPO_NAME} sh ${SOURCE_DIR}/push_steve_to_github.sh
